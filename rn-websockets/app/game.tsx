...
export default function CPRPracticeGame() {
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      const newDataPoints = event.data.split(" ").map((depth: string) => ({
        depth: Math.max(0, MAX_DISTANCE - Math.min(100, +depth) - 40),
        date: new Date(),
      }));

      setDataPoints((prev) => [...prev.slice(-200), ...newDataPoints]);
    };
  ...
  function countCompressions(): number {
    const compressionThreshold = 30; // mm, realistic minimum to consider a valid compression
    let inCompression = false;
    let count = 0;

    const slice = dataPoints.slice(-200);

    for (let i = 0; i < slice.length; i++) {
      const depth = slice[i].depth;

      if (depth >= compressionThreshold && !inCompression) {
        // Starting a compression
        inCompression = true;
      } else if (depth < compressionThreshold && inCompression) {
  ...
  function calculatePoints(depth: number, frequency: number): number {
    ...
    if (
      frequency >= OPTIMAL_FREQUENCY_MIN &&
      frequency <= OPTIMAL_FREQUENCY_MAX
    ) {
      frequencyPoints = 50; // Perfect frequency
    } else if (frequency < OPTIMAL_FREQUENCY_MIN) {
      frequencyPoints = Math.max(
        0,
        50 - (OPTIMAL_FREQUENCY_MIN - frequency) * 5
      ); // Lose 5 points per compression below min
    } else {
      frequencyPoints = Math.max(
        0,
        50 - (frequency - OPTIMAL_FREQUENCY_MAX) * 5
      ); // Lose 5 points per compression above max
    }
    ...
    if (time > 0) {
      ...
      if (!modalVisible && !showResults) {
        const timeout = setTimeout(() => {
          setTime(time - 1);
          const currentFrequency = countCompressions() * 6;
        ...
        <ScrollView contentContainerStyle={styles.content}>
          <View style={styles.statsCard}>
            ...
            <View style={styles.stat}>
              ...
              <Text style={styles.statLabel}>{"Seconds\nRemaining"}</Text>
            </View>
            <View style={styles.divider} />
            <View style={styles.stat}>
              ...
              <Text style={styles.statLabel}>{"Compressions\nPer Minute"}</Text>
            </View>
          </View>
          <ChartCard
        ...
        >
          <View style={styles.modalTextContainer}>
            {currentPlayerIndex === 0 ? (
              <>
                <ModalText>
                  Welcome! Now that you have gone through the training material,
                  you are ready to practice CPR with your simulation device.
                </ModalText>
                <ModalText>
                  When you are ready, press begin. You will have 30 seconds, and
                  you will be scored based on the accuracy of your CPR.
                </ModalText>
          ...
          <View style={styles.resultsContainer}>
            {players.map((player, index) => {
              ...
              return (
                <View
                  key={index}
                  style={[styles.resultRow, isWinner && styles.winnerRow]}
                >
                  <View style={styles.resultContent}>
                    ...
                    <Text
                      style={[styles.resultName, isWinner && styles.winnerName]}
                    >
                      {player.name}
                    </Text>
                  </View>
